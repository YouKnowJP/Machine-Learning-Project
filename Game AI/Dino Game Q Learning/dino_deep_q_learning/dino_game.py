...
"""
Modified version of the original dino game to be used with the ML agent.
Some adjustments have been made to make it suitable for ML training.
"""
...
class Bird:
    """Bird obstacle with variable speed offset and two different types requiring different actions."""
    ...
    def __init__(self, x):
        ...
        if self.condition == "jump":
            # Small, low bird that the agent must jump over
            self.width = 40
            self.height = 50
            self.y = GROUND_LEVEL - self.height - 10  # Bird flies close to ground
        else:
            # Larger, high bird that the agent must duck under
            self.width = 60
            self.height = 200
            self.y = GROUND_LEVEL - self.height - 40  # Bird flies higher up where agent would hit it unless ducking

    def update(self, game_speed):
        """Update bird position based on game speed and individual speed offset."""
        self.x -= game_speed + self.speed_offset

    def draw(self, surface):
        # Different colors for different bird types
        if self.condition == "jump":
            color = RED
        else:
            color = BLUE
